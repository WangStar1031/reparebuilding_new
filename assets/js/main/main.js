	...
	for( var i = 0; i < curInfo.arrNodes.length; i++){
		...
		strHtml += '<div style="padding: 5px; border-bottom:1px solid #333333; margin-bottom:10px;" id="imgId' + curNode.Id + '">';
			strHtml += '<table style="width:100%;">';
				strHtml += '<tr>';
					strHtml += '<td style="width: 10%; vertical-align: text-bottom; text-align: center;">';
					...
					strHtml += '</td>';
					strHtml += '<td style="width:90%; vertical-align:top;">';
						strHtml += '<table>';
							strHtml += '<tr>';
								strHtml += '<td>';
									strHtml += '<div class="photocontainer-title" style="float: right; margin-right: 10px; font-weight: 700;"> ' + curNode.info.ShootingDate + '</div>';
									strHtml += '<div class="photocontainer-title" style="float: right;">Date Taken : </div>';									
	...
	$.post("api_process.php", {action: "getUploadedInfo", projectName: "project1", apartNo: apartNo, idxPhoto: _id, catPhoto: _cat}, function(data){
		...
		for( var i = 0; i < infos.length; i++){
			...
			strHtml += '<div class="groupContainer" style="top:' + top + 'px;left:' + left + 'px; position: absolute;" groupId="' + curInfo.groupId + '">';
				...
				strHtml += '<div class="photoCount" onclick="countClicked(' + i + ')"><p>' + ptCount + '</p></div>';
			strHtml += '</div>';
		}
	...
	fr.onload = function(e) {
		...
		img.onload = function(){
			if( e.total >= 350000 || img.width > 1600 || img.height > 1600){
				...
				var strHtml = "";
					strHtml += '<img class="uploadedImg" style="width:36px; height:36px;" src="' + imgReduced + '">';
				...
				var strHtml = "";
				if( fullWidth < 769 ){		  
					strHtml += '<img class="uploadedImg" style="width:36px; height:36px;" src="' + target.src + '">';
		        } else{
					strHtml += '<img class="uploadedImg" style="width:100px; height:100px;" src="' + target.src + '">';
				}	
				$(".uploadedImgPan").html(strHtml);
				$(".uploadedImgPan").css({"top":"10px", "left":"10px"});
				$(".uploadedImgPan").draggable();
